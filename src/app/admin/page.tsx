      ...
      <div className="product-shell admin-shell">
        ...
        <section className="product-main">
          ...
          <div className="product-content">
            ...
            {section === "invoices" && <EditableListSection title="Счета и оплаты" description="Выставляйте сумму, срок и меняйте статус после проверки чека." addLabel="Выставить счёт" icon={<CircleDollarSign />} items={snapshot.invoices} getTitle={(item) => item.title} getMeta={(item) => `${projectName(snapshot, item.projectId)} · оплатить до ${formatDate(item.dueAt)}`} getDetail={(item) => `${formatMoney(item.amount)} · ${item.status}`} onCreate={() => openModal("invoices")} onEdit={(item) => openModal("invoices", item as unknown as Record<string, unknown>)} />}
...
function EditableListSection<T extends { id: string }>({ title, description, addLabel, icon, items, getTitle, getMeta, getDetail, onCreate, onEdit }: { title: string; description: string; addLabel: string; icon: React.ReactNode; items: T[]; getTitle: (item: T) => string; getMeta: (item: T) => string; getDetail: (item: T) => string; onCreate: () => void; onEdit: (item: T) => void }) {
  return <><SectionHeading title={title} description={description} action={addLabel} onAction={onCreate} />{items.length ? <div className="card admin-record-list">{items.map((item) => <article className="admin-record-row" key={item.id}><span className="activity-icon">{icon}</span><div><strong>{getTitle(item)}</strong><span>{getMeta(item)}</span><p>{getDetail(item) || "Комментарий не добавлен"}</p></div><button className="edit-button" onClick={() => onEdit(item)}><Pencil size={14} /> Изменить</button></article>)}</div> : <EmptyAdminState title="Записей пока нет" text={description} action={addLabel} onAction={onCreate} />}</>;
}
