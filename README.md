...
- русскоязычный лендинг: холодная серо-голубая палитра, стеклянные карточки, градиентные заголовки и адаптивная сетка;
...
Кабинет работает только с реальными данными из подключённого Google Apps Script. Публичного демонстрационного входа нет.

Публичный дизайн изолирован в `src/app/cold-landing.css` и не меняет стили кабинета и админки. Интерактивное превью кабинета показывает явно обозначенные условные данные; реальные кейсы загружаются через существующий API. Анимации учитывают `prefers-reduced-motion`, вкладки доступны с клавиатуры.
...
Workflow подставляет эти переменные в production-сборку автоматически. Если переменные не заданы, вход и админка показывают понятное сообщение о незавершённом подключении.
