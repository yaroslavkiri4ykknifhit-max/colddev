...
function route(input) {
  switch (input.action) {
    ...
    case 'public.portfolio': return publicPortfolio();
...
function clientLogin(input) {
  ...
  const storedHash = String(project.access_hash || '').trim().toLowerCase();
  const normalizedCode = normalizeAccessCode(accessCode);
  const validCode = storedHash && (hash(accessCode) === storedHash || hash(normalizedCode) === storedHash);
  if (!validCode) throw new Error('Код доступа не совпал. Откройте проект в админке и установите новый код.');
...
function parseImageUrls(value) { const raw = String(value || '').trim(); if (!raw) return []; try { const parsed = JSON.parse(raw); if (Array.isArray(parsed)) return parsed.map(item => String(item).trim()).filter(Boolean); } catch (error) {} return raw.split(/\r?\n|,/).map(item => item.trim()).filter(Boolean); }
function normalizeImageUrls(value) { return Array.isArray(value) ? value.map(item => String(item).trim()).filter(Boolean) : parseImageUrls(value); }
function normalizeAccessCode(value) { return String(value || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, ''); }
...
function normalizeForSheet(entity, value) { const now = new Date().toISOString(); if (entity === 'clients') return { id: value.id, name: value.name, company: value.company, phone: value.phone, telegram: value.telegram, email: value.email, status: value.status || 'Активен', created_at: value.created_at || now }; if (entity === 'projects') return { id: value.id, client_id: value.clientId || value.client_id, name: value.title || value.name, type: value.type, status: value.status || 'В работе', progress: value.progress || 0, started_at: value.startedAt || now, deadline: value.deadline, site_url: value.siteUrl, preview_url: value.previewUrl, current_action: value.currentAction, manager_comment: value.managerComment, last_updated_at: now, access_hash: value.accessCode ? hash(normalizeAccessCode(value.accessCode)) : (value.accessHash || '') }; if (entity === 'stages') return { id: value.id, project_id: value.projectId || value.project_id, title: value.title, description: value.description, order: value.order || 0, status: value.status || 'waiting', started_at: value.startedAt, completed_at: value.completedAt }; if (entity === 'updates') return { id: value.id, project_id: value.projectId || value.project_id, title: value.title, description: value.description, date: value.date || now.slice(0, 10), category: value.category || 'Общее', image_url: value.imageUrl, link_url: value.linkUrl }; if (entity === 'ads') return { id: value.id, project_id: value.projectId || value.project_id, period: value.period || value.category, impressions: value.impressions || 0, clicks: value.clicks || 0, spend: value.spend || 0, leads: value.leads || 0, budget_left: value.budgetLeft || 0, comment: value.comment || value.description, screenshot_url: value.screenshotUrl }; if (entity === 'invoices') return { id: value.id, project_id: value.projectId || value.project_id, title: value.title, amount: value.amount || 0, created_at: value.createdAt || now, due_at: value.dueAt || value.deadline, status: value.status || 'Ожидает оплаты', comment: value.comment, receipt_file_id: value.receiptFileId, receipt_name: value.receiptName }; if (entity === 'services') return { id: value.id, title: value.title, description: value.description, price: value.price, price_mode: value.priceMode || 'fixed', button_label: value.buttonLabel || 'Обсудить', image_url: value.imageUrl, active: value.active === undefined ? true : value.active, order: value.order || 0 }; if (entity === 'portfolio' || entity === 'cases') { const imageUrls = normalizeImageUrls(value.imageUrls !== undefined ? value.imageUrls : value.imageUrl); return { id: value.id, kind: entity === 'cases' ? 'case' : 'portfolio', title: value.title, category: value.category, description: value.description, result: value.result, image_url: JSON.stringify(imageUrls), url: value.url, published: value.published === undefined ? true : value.published, order: value.order || 0 }; } return Object.assign({}, value); }
...
function mapPortfolio(row) { const imageUrls = parseImageUrls(row.image_url).map(resolveImageUrl); return { id: row.id, kind: row.kind, title: row.title, category: row.category, description: row.description, result: row.result, imageUrl: imageUrls[0] || '', imageUrls: imageUrls, url: row.url, published: String(row.published) !== 'false', order: Number(row.order || 0) }; }
function resolveImageUrl(value) { const url = String(value || '').trim(); if (!/^https?:\/\/ibb\.co\//i.test(url)) return url; try { const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true, followRedirects: true }); const html = response.getContentText(); const match = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)/i) || html.match(/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:image["']/i); return match ? decodeHtmlEntities(match[1]) : url; } catch (error) { return url; } }
function decodeHtmlEntities(value) { return String(value || '').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#x2F;/gi, '/').replace(/&#47;/g, '/'); }
