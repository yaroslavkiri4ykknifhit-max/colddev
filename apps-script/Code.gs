...
function mapPortfolio(row) { const imageUrls = parseImageUrls(row.image_url); return { id: row.id, kind: row.kind, title: row.title, category: row.category, description: row.description, result: row.result, imageUrl: imageUrls[0] || '', imageUrls: imageUrls, url: row.url, published: String(row.published) !== 'false', order: Number(row.order || 0) }; }
