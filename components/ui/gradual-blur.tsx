...
const useResponsiveDimension = <T,>(responsive: boolean | undefined, config: any, key: string): T => {
  const [value, setValue] = useState<T>(config[key]);
  useEffect(() => {
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const responsiveHeight = useResponsiveDimension<string>(config.responsive, config, 'height');
  const responsiveWidth = useResponsiveDimension<string | undefined>(config.responsive, config, 'width');
